...
DEADZONE_PX = 5                      # Pixels — must match Arduino DEADZONE

# --- THROTTLE ---
SEND_INTERVAL = 0.033                # ~30 Hz
last_send_time = 0.0

cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)  # CAP_DSHOW = faster startup on Windows
...
while True:
    ...
    # Plate boundary circle — red if out of bounds, yellow otherwise
    ...
    # Green filled dot when ball is centered
    if centered:
    ...
    # ------------------------------------------------------------------ #
    # 3. SEND DATA TO ARDUINO  (throttled to ~30 Hz)
    # ------------------------------------------------------------------ #
    now = time.time()
